class Unit extends DataBase
{
	...
	public String toString()
	{
		...
		if (hp<=0)
			return ".";
		return indicativeChar+"["+allignToMiddle(""+hp,"]",4)+mpBar;//+lastHit;
	}
	public String hpBar()
	{
		int scale = 8;
		...
		if (maxHp>1000)
			scale=9;
		return scaleBar((double)hp/(double)maxHp,'*', scale);
	}
	...
	{
		double def=stability;
		def/=100.0;

		hp-=(double)damage*(1-def);
		
	}
	public void deductMp()
	{
		if (splash==1)
			mp-=8;
		if (splash==2)
	...
	public void modifyLastHit(int target)
	{
		lastHit=target;
	}
	private String scaleBar(double percentage, char dot, int scale)
	{
		String bar = "";
		
		percentage = (int)Math.round(percentage*scale);
		if (percentage<0)
			percentage=0;
		for (int i = 0;i<percentage ;i++ )
			bar = bar+dot;
		for (int i = 0;i<(scale-percentage) ;i++ )
			bar = bar+"_";
		return bar;
	}
...
class Leg
{
	...
	public String toString()
	{
		return pad("  <"+name+">", 18)+pad("type= "+(air?"air,":"ground,")+(melee?"melee":"support"),23)+pad("stability= "+stability,15)+pad("maxLoad= "+maxLoad,15)+pad("watt= "+watt,15)+"rec="+(air?1.7:1)*(melee?1:1.3)*(Math.pow(maxLoad,1.1)*Math.pow((1+(double)stability/100.0),1.1));
	}
	public String getName()
	{
		return name;
...
class Body
{
	...
	public String toString()
	{
		return pad("  <"+name+">", 18)+pad("hp= "+maxHp,12)+pad("mp= "+maxMp,12)+pad("type= "+armType,15)+pad("weight= "+load,12)+pad("watt= "+watt,12)+"rec="+(((maxHp/3)-(load))*(double)(1.0+Math.pow((double)maxMp/50.0,0.2)));
	}
	public String getName()
	{
		return name;
