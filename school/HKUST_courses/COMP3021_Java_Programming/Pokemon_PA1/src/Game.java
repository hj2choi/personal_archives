...
public class Game {
	...
	public static final int REPEAT_COUNT=20000;
	public static final long TIME_LIMIT_MILLIS = 50000;
	...
	public static void main(String[] args) throws Exception{
		...
		for (int j=1; j<REPEAT_COUNT; ++j) {
			...
			if (j>lastBreakthrough+10) {
				...
				game.maps[j].get(blockCell.getY(), blockCell.getX()).setPathCost(Cell.WALL_PATH_SCORE);
				//System.out.println("blocking cell: "+blockCell);
			}
			
			game.shuffleTargets();
			int currentScore = game.playGame(j);
			if (currentScore > maxScore && (game.players[j].getX()==destX && game.players[j].getY()==destY)) {
				bestPlayerIndex=j;
