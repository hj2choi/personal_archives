...
#End Region
    ...
    Overrides Function Process(ByRef image As Bitmap, ByVal x As Integer, ByVal y As Integer) As Color
        ...
        For i As Integer = y - cy To (y - cy) + kernel_height - 1
            For j As Integer = x - cx To (x - cx) + kernel_width - 1
                red += image.GetPixel(Math.Min(Math.Max(j, 0), image.Width - 1), Math.Min(Math.Max(i, 0), image.Height - 1)).R * kernel(j - (x - cx), i - (y - cy))
                green += image.GetPixel(Math.Min(Math.Max(j, 0), image.Width - 1), Math.Min(Math.Max(i, 0), image.Height - 1)).G * kernel(j - (x - cx), i - (y - cy))
                blue += image.GetPixel(Math.Min(Math.Max(j, 0), image.Width - 1), Math.Min(Math.Max(i, 0), image.Height - 1)).B * kernel(j - (x - cx), i - (y - cy))
                divisor += kernel(j - (x - cx), i - (y - cy))
            Next
        Next
        red /= divisor
        green /= divisor
