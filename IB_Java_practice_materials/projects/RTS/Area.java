...
//Library :                   в┤ вч ид ио е╪ 
	...
	{
		clearscreen();
		System.out.println("Watt : "+MainMenu.player[occupiedBy].getWatt()+"\n\n\n");
		//System.out.println("Income : "+MainMenu.player[occupiedBy].getIncome()+"\n\n\n");
		...
		if (areaStatus==0)
			...
			System.out.println("Mining Zone");

		System.out.println(pad("",30)+"Army size : "+armySizeInArea);
		System.out.println("\n\n\n"+pad("",30)+"(Army status)\n\n\n\n");
		System.out.println("---------------------------------------------------------------------------------------------------");
	...
	public void defeatedBy(Unit[] newArmy, int newArmySize, int playerNumber)
	{
		...
		for (int i=0;i<newArmySize ;i++ )
		{
			armyInArea[i] = new Unit();
			armyInArea[i].copy(newArmy[i]);
		}
		armySizeInArea = newArmySize;
	}
	public void clearArmyInArea()
	{
		for (int i=0;i<armySizeInArea ;i++)
			armyInArea[i] = new Unit();
		armySizeInArea = 0;
		chkUpdate();
	}
	public void buildUnit(Player player, int choice) // build unit in game (haven't work out watt yet)
	{
		try
		{
			if (player.getConstructedUnit(choice).getHp()>0 && Map.countMeleeUnits(armyInArea, armySizeInArea)<10 && Map.countSupportUnits(armyInArea, armySizeInArea)<10)
			{
