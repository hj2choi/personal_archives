public class CharacterStat extends wadv
{
	public String hebar(double gg, char dtt)
	{
		...
		main.mp = ((long)((main.lv*main.lv/8)/4+main.lv*2+(Math.sqrt(main.intel)*main.intel)*1.2 + main.intel*4 + main.wep_stat[8] + 10))*1.8;
		if (main.intel>14)
			main.mp=main.mp+(main.intel-14)*3+((main.intel-14)*(main.intel-14)/4);
		main.Gamehp = main.hp;			// hp.mp in battle
		main.Gamemp = main.mp;
		main.atkmax = (long)((main.str*(main.str/7))*(main.wep_stat[0]/20)+(main.str*(main.wep_stat[0]/20))+Math.sqrt(main.dex*(main.dex/30))+main.sta/5);// + (dex*(dex/10)-1) + (sta/4) + ((wep_stat[0])/4)-str);		//battle statistics
		main.atkmin = (long)((main.atkmax/4.5)+(main.dex*(main.dex-1)/7)*(main.wep_stat[0]/20)+2)+1+dex/2;
		...
		main.def = (long)(main.str/10+main.sta/3+(main.sta*Math.sqrt(Math.sqrt(main.sta))/40)+main.wep_stat[2]);
		main.acc = 90-main.difficulty+main.dex+dex-Math.sqrt(dex)+main.wep_stat[9];				// use with random(range = 0~100) (hit is determined when (num is 1~acc))
		main.speed = (20+dex/1.5+(dex-(Math.sqrt(dex))));
		if (main.callClassType==2 || main.callClassType==3)
		{
			...
			System.out.println("Power:		"+(int)main.spatk);
			System.out.println("Defence:	"+(int)main.def);
			System.out.println("Accuracy:	"+(int)main.acc);
			System.out.println("Attack Speed:	"+(int)main.speed);
			System.out.println("");
			...
			if (main.callClassType==3){System.out.print("[4]	");}System.out.print("Enduarance:	"+(int)main.sta+"	");	 if (main.sta-main.sd!=0){System.out.println("(+"+(int)(main.sta-main.sd)+")");}else{System.out.println("");}
			if (main.callClassType==2)
			{
				System.out.println("");
				System.out.println("		Wand:		"+main.wep);
				System.out.println("		Armor:		"+main.arm);
				System.out.println("		Accessory:	"+main.head);
				System.out.println("");
			}
		}
