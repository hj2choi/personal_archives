class Euler059 extends HongJoonLibrary
{
			...
			for (int j=1; j<26; j++)
				for (int k=1; k<26; k++)
				{
					...
					for (int a=0; a<message.length; a++)
						message[a]=XOR(cypher[a], key[a%3]);
					isPlainText=true;
					ASCIISum=0;
					for (int a=0; a<message.length; a++)
					{
						...
						if (message[a]<32 || message[a]>122)
							isPlainText=false;
					}
					if(isPlainText)
						for (int a=0; a<message.length; a++)
							System.out.print((char)message[a]);
					if (isPlainText)
					{
